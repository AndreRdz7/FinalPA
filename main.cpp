...
void giveCardsAtStart(std::vector<Card>& player,  std::vector<Card>& drawPile) {
	for (int i = 0; i < 7; i++)
	{
		Card drawnCard = drawPile.at(0);
		player.insert(player.begin(), drawnCard);
...
void selectOptionOfMenu(int numberOfPlayer, std::vector<Card> actualPlayerDeck) {
	int choice;
	printMenu();
	...
	case 1:
		...
		// add function to draw card
		drawCardAndPlay();
		// draw card function() and in authomatic run function ifCardWorksPlayIt()
		// finishTurn()
		break;
	...
	case 4:
		...
		printInstructivo();
		selectOptionOfMenu(numberOfPlayer, actualPlayerDeck);
		break;
...
int main(int argc, char const* argv[])
{
	...
	while (gameIsRunning(playerOne, cpuOne, cpuTwo)) {
		...
		selectOptionOfMenu(turn, actualV);
	}
	return 0;
}